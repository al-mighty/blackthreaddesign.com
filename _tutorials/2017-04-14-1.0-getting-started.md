---
layout: single
title:  "Black Thread three.js tutorials - 1.0 Getting Started"
#tags: Test
...
In this first tutorial we'll cover the simplest possible method of getting three.js to display something in your browser, along with a short tour of the basic elements making up a three.js app. 
...
Next, we'll include the latest version of three.js, which at the time of writing is r84. If a later version has been released, change the number below.
...
Here we have created a [WebGLRenderer](http://threejs.org/docs/#api/renderers/WebGLRenderer) and set it to 100% of the browser window's width and height. This will not be automatically resized if we resize the window, but we'll cover how to fix that later.  
...
5. The [SoftwareRenderer](http://threejs.org/examples/#software_geometry_earth). This generates the image in software (i.e not using any of the above APIs) and then writes the resulting 2D image to a Canvas element. 
...
The other thing to take note of here is the format of the color - it's a [Hex Triplet](https://en.wikipedia.org/wiki/Web_colors#Hex_triplet) - a six digit (three byte) hexadecimal number representing an RGB color. You may be more familiar with the CSS form `#b300b3` which is identical to `0xb300b3`. Here, `0x` tells JavaScript that this is a hex number, then the first two digits `b3` represent the red component of the color, the next two `00` are the green component and the final two, `b3` again, are the blue component. Each are in the hex range `00 -> ff` which is equivalent to `0 -> 256` in binary. 

There are lots other ways of specifying colors in three.js - see the [Color](threejs.org/docs/#api/math/Color) docs for details - but the hex triple is the recommended method and the one we'll be sticking with in these tutorials. 
...
Wait a minute! We just said that the DirectionalLight doesn't have a position in space, and then we went right ahead and set the light's position! 
...
And here's a the the same code converted to ES6 - no major changes yet, just the `var`s are changed to `const`s.
